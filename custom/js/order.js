...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			//} // /else
			// function lettersOnlyCheck(clientName){
			// var letter = /^[a-Za-z]+$/;
			if(clientName == "") {
...
function discountFunc() {
	var discount = $("#discount").val();
	 ...
	 //var discount = $("#discount").val();
	 if(discount>=(0.1*totalAmount)) {
		 $("#discount").after('<p class="text-danger">Discount cannot be more than 10% </p>');
		 $('#discount').closest('.form-group').addClass('has-error');
	 }
	 else if(discount<=(0.1*totalAmount)){
